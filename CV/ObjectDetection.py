...
def get_landmarks(frame, pose_result):
    ...
    if lm[mp.solutions.pose.PoseLandmark.LEFT_WRIST].visibility > 0.5:
        wrists.append((int(lm[mp.solutions.pose.PoseLandmark.LEFT_WRIST].x * w),
                        int(lm[mp.solutions.pose.PoseLandmark.LEFT_WRIST].y * h)))

    if lm[mp.solutions.pose.PoseLandmark.RIGHT_WRIST].visibility > 0.5:
        wrists.append((int(lm[mp.solutions.pose.PoseLandmark.RIGHT_WRIST].x * w),
                        int(lm[mp.solutions.pose.PoseLandmark.RIGHT_WRIST].y * h)))

    if lm[mp.solutions.pose.PoseLandmark.LEFT_EAR].visibility > 0.5:
        ears.append((int(lm[mp.solutions.pose.PoseLandmark.LEFT_EAR].x * w),
                      int(lm[mp.solutions.pose.PoseLandmark.LEFT_EAR].y * h)))

    if lm[mp.solutions.pose.PoseLandmark.RIGHT_EAR].visibility > 0.5:
        ears.append((int(lm[mp.solutions.pose.PoseLandmark.RIGHT_EAR].x * w),
                      int(lm[mp.solutions.pose.PoseLandmark.RIGHT_EAR].y * h)))
...
def run_detect_object(model_path="NewFullDataSet_FT.pt", ):
    ...
    # ================= CONFIG =================
    FORBIDDEN = ["mobile", "book", "watch", "headphones", "sunglass", "earbuds", "laptop", "face_mask","calculator"]

    SUNGLASS_THRESHOLD = 0.60
    FACE_MASK_THRESHOLD = 0.66
    EARBUDS_THRESHOLD =0.54
    ...
    cap  = cap = cv2.VideoCapture("WIN_20260128_04_25_54_Pro.mp4")
    ...
    while True:
        ...
        wrists, ears = get_landmarks(frame,
                                     pose_result)
        ...
        for box in results[0].boxes:
            ...
            if refined_label in detected_once:
                continue
            ...
            # adjusted_threshold = min(max(adjusted_threshold, 0.5), 0.9)

            # ================= INSTANT DETECT =================
            if refined_label in ["watch","book","headphones","calculator","earbuds","laptop"] and (
                     conf >= adjusted_threshold ):  #or refined_label != label
                color = (0, 0, 255)
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                cv2.putText(frame, f"{refined_label} {conf:.2f}", (x1, y1 - 10),
            ...
            # ================= DELAY DETECT =================
            DELAY_SECONDS = SUNGLASS_DELAY if refined_label == "sunglass" else DELAY_SECONDS
